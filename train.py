...
def prepare_batch_for_model(batch, dataset: KGProcessedDataset, save_batch=False):
    ht_tensor, r_tensor, entity_set, entity_feat, indeg_cats, outdeg_cats, queries, labels, r_queries, r_relatives, h_or_t_sample = batch
    if entity_feat is None:
        entity_feat = torch.from_numpy(dataset.entity_feat[entity_set]).float()

    batch = ht_tensor, r_tensor, entity_set, entity_feat, indeg_cats, outdeg_cats, queries, labels, r_queries, r_relatives, h_or_t_sample
    if save_batch:
...
def move_batch_to_device(batch, device):
    ...
    indeg_cats = indeg_cats.to(device)
    outdeg_cats = outdeg_cats.to(device)
...
def train(global_rank, local_rank, world):
    ...
    for epoch in range(start_epoch, FLAGS.epochs):
        for i, batch in enumerate(tqdm(train_loader)):
            ...
            ht_tensor, r_tensor, entity_set, entity_feat, indeg_cats, outdeg_cats, queries, labels, r_queries, r_relatives, h_or_t_sample = batch
            scores = ddp_model(ht_tensor, r_tensor, r_queries, entity_feat, r_relatives, h_or_t_sample, indeg_cats, outdeg_cats, queries)
    ...
    with torch.no_grad():
        for i, (subbatches, t_correct_index, t_filter_mask) in enumerate(tqdm(dataloader)):
            ...
            for subbatch in subbatches:
                ...
                ht_tensor, r_tensor, entity_set, entity_feat, indeg_cats, outdeg_cats, queries, _, r_queries, r_relatives, h_or_t_sample = subbatch
                subbatch_preds = model(ht_tensor, r_tensor, r_queries, entity_feat, r_relatives, h_or_t_sample, indeg_cats, outdeg_cats, queries)
